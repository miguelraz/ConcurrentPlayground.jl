...
The different versions are in different folders, starting with `mpmpc0.jl`. Each implementation describes the tweaks from the previous versions and the output when run.

You can try and look at the code and figure out what went wrong before reading the diagnosis here.
...
Which will probably look different on your machine, but the behaviour is the following: all producers and consumers rev up, but none start doing work.
